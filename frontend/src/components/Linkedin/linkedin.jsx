...
const Linkedin = ({connectLinkedinStatus}) => {
  ...
  const clientId = '77e5zc91lfplxv';
    ...
    const getProfile = async (token)=> {
         const response = await axios.get('http://localhost:3300/api/linkedin/get-profile', 
    {
        headers: {
            token,
        }   
...
//       console.log(response.data);
    // Post an article
    try {
      const response = await axios.post('http://localhost:3300/api/linkedin/post-article', 
    ...
    if (code) {
      ...
      // Replace the URL with your backend URL
        const response = await axios.post('http://localhost:3300/api/linkedin/exchange-token', 
             {
                code,
                 redirectUri,
                token
    ...
    <div>
    <OAuthPopup
  url={`https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=r_liteprofile%20r_emailaddress%20w_member_social`}
...
>
  <button>Connect With Linkedin</button>
            ...
            <br></br>
            {connectLinkedinStatus && (<> <textarea value={article} onChange={(e) => setArticle(e.target.value)} /><br/>
